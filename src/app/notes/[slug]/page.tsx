...
async function getNote(uid: string) {
  const client = createClient();
  try {
    return await client.getByUID("note", uid);
  } catch (error) {
    console.error(`Failed to fetch note ${uid}:`, error);
    return null;
  }
}
...
export async function generateMetadata({
  ...
  const title = `${note.data.title} · ${siteConfig.author}`;
  const description = note.data.excerpt || note.data.title;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: "article",
      publishedTime: note.first_publication_date,
      url: `/notes/${slug}`,
      authors: [siteConfig.author],
      images: note.data.featured_image?.url
        ? [
            {
              url: note.data.featured_image.url,
              width: note.data.featured_image.dimensions?.width,
              height: note.data.featured_image.dimensions?.height,
    ...
    twitter: {
      ...
      title,
      description,
      images: note.data.featured_image?.url ? [note.data.featured_image.url] : [],
    },
  };
}
    ...
    return (
      <main className="px-7 py-10 flex-1">
        <div className="prose m-auto">
          ...
          <p>Sorry, we couldn't find this note.</p>
          <BackLink href="/notes" />
        </div>
      </main>
    );
    ...
    <main className="px-7 py-10 of-x-hidden">
      <div className="post-header prose m-auto mb-8" lang="en">
        ...
        <p className="opacity-50 mt-2 slide-enter-50">
          {formatDate(note.first_publication_date)}
        </p>
      </div>
...
function reconstructContent(
  ...
  for (const block of blocks) {
    ...
    if (trimmed.startsWith("```")) {
      ...
      // If a single Prismic block contains multiple lines (soft breaks),
